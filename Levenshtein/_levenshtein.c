                   ...
                   size_t *lensum)
{
  ...
  if (PyObject_TypeCheck(arg1, &PyString_Type)
    ...
    *lensum = len1 + len2;
    ...
    len1 = PyList_GET_SIZE(arg1);
    len2 = PyList_GET_SIZE(arg2);
    *lensum = len1 + len2;

    string1 = calloc(sizeof(*string1), len1);
    if (string1 == NULL) {
      PyErr_NoMemory();
      return -1;
    }
    string2 = calloc(sizeof(*string2), len2);
    if (string2 == NULL) {
      PyErr_NoMemory();
      return -1;
    }
    ...
    {
      size_t d = lev_l_edit_distance(len1, string1, len2, string2, xcost);
      if (d == (size_t)(-1)) {
        PyErr_NoMemory();
        return -1;
      }
